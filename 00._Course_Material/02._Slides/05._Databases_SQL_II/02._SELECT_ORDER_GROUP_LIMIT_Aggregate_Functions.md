...
**Assignment 1**: Get the first 5 games.

**Assignment 2**: Get the first game that has a price of over 5000.
...
`"A%"`: Word starts with an `A` / `a`.

`"%a"`: Word ends with an `A` / `a`.

**Assignment**: 

1. Get the game with a title that starts with an `S`.
