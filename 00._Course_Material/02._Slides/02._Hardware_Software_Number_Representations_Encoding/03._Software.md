...
This diagram is one way to view the relation between hardware and software. 

Another way to describe it is a top-down view where the kernel sits on top of the hardware with the shell and programs utilizing the kernel to provide machine instructions. 
...
`$$` is an alias for the terminal process.
